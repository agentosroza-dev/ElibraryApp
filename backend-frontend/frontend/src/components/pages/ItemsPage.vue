            ...
            <div class="container-fluid">
                <div class="d-flex flex-wrap justify-content-between align-items-center page-header">
                    <div class="mb-2 mb-md-0">
                        <h3 class="hero-title mb-1">List All Books</h3>
                    </div>
            ...
            <div class="container-fluid">
                ...
                <div class="card shadow-sm border-0 books-table-card">
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover align-middle mb-0 books-table">
                                ...
                                <tr>
                                    ...
                                    <th style="width: 40px">Total Read</th>
                                ...
                                <tr v-for="(item, index) in items.data" :key="item.id">
                                    ...
                                    <td>
                                        <div class="book-cell">
                                            ...
                                            <div class="book-meta">
                                                <div class="book-title text-truncate">{{ item.title }}</div>
                                                <div class="book-slug text-truncate">{{ item.slug || "—" }}</div>
                                            </div>
                                        </div>
                                    </td>

                                    <td>
                                        <span v-if="item.category" class="badge badge-category">
                    ...
                    <div
                        ...
                        class="card-footer bg-white border-0 d-flex flex-wrap justify-content-between align-items-center"
                    >
                                        ...
                                        <div class="form-row">
                                            ...
                                            <div class="form-group col-md-12">
                                                ...
                                                <select
                                                    ...
                                                    class="form-control tag-multi-select"
                                                    multiple
                                                >
                                                    <option v-for="tag in tags" :key="tag.id" :value="Number(tag.id)">
                                                        {{ tag.name }}
                                                    </option>
                                                </select>
                                                ...
                                                <small v-if="errors.tag_ids" class="text-danger d-block">
                                                    {{ errors.tag_ids[0] }}
                                                </small>
                                    ...
                                    <div class="form-row">
                                        ...
                                        <div class="form-group col-md-12">
                                            <div class="custom-control custom-switch mt-2">
                                                ...
                                                />
                                                <label class="custom-control-label" for="item_active">
                                                    Active item
                                                </label>
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group col-md-6">
                                            ...
                                            <div v-if="coverPreview" class="preview-box mt-3">
                                                <small class="text-muted d-block mb-2">
                                                    {{ coverFile ? "New cover preview" : "Current cover" }}
                                                </small>
                                                <img :src="coverPreview" class="cover-preview-image" alt="cover preview" />
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group col-md-6">
                                            ...
                                            />
                                            <small class="text-muted d-block">
                                                Allowed: PDF • Max: 51MB
                                            </small>
                                            <small v-if="errors.file" class="text-danger">{{ errors.file[0] }}</small>

                                            <div v-if="itemForm.id && existingFileUrl && !bookFile" class="mt-3">
                                                <small class="text-muted d-block mb-2">Current file</small>
                                                <a
                                                ...
                                                >
                                                    ...
                                                    {{ existingFileName || "Open file" }}
                                                </a>
                                            </div>

                                            <div v-if="selectedFileName" class="file-pill mt-3">
...
const selectedTags = computed(() => {
    const tagIds = new Set((itemForm.tag_ids || []).map(Number));
    return tags.value.filter((tag) => tagIds.has(Number(tag.id)));
});
...
function getAuthorName(item) {
    ...
    if (item?.author_id && userMap.value[String(item.author_id)]?.name) {
        return userMap.value[String(item.author_id)].name;
    }
    return "—";
}
...
function toAssetUrl(value, type = "generic") {
    ...
    if (raw.startsWith("public/")) {
        return `${getOrigin()}/${raw.replace(/^public\//, "")}`;
    }

    if (type === "cover" || type === "file") {
        return `${getOrigin()}/storage/${raw.replace(/^\/+/, "")}`;
    }
...
async function loadViewsForCurrentPage() {
    ...
    const results = await Promise.allSettled(
        missingIds.map((id) => apiGetBookViewCount(id))
    );
    ...
    results.forEach((result, index) => {
        ...
        if (result.status === "fulfilled") {
            nextMap[itemId] = Number(result.value?.data?.views_count ?? 0);
        } else {
            nextMap[itemId] = 0;
        }
...
function openEdit(item) {
    ...
    selectedAuthorName.value = authorName !== "—" ? authorName : "";
    authorSearch.value =
...
async function loadItems() {
    ...
    try {
        const res = await apiGetItemsAdmin({ ...filters });
...
function onCoverChange(event) {
    ...
    coverPreview.value = file
        ...
        : toAssetUrl(itemForm.cover_url, "cover") || "";
}
...
function selectHighlightedAuthor() {
    if (
        ...
        highlightedAuthorIndex.value < users.value.length
    ) {
